          ...
          if (data.length > 0) {
            data.forEach((element) => {
              if (element.ProjectStart != null) {
                element.ProjectStart = moment(new Date(element.ProjectStart)).format('YYYY-MM-DD')
              }
              if (element.ProjectEnd != null) {
                element.ProjectEnd = moment(new Date(element.ProjectEnd)).format('YYYY-MM-DD')
              }
        ...
        .then((response) => {
          ...
          data.forEach((element) => {
            if (element.StageStart != null) {
              element.StageStart = moment(new Date(element.StageStart)).format('YYYY-MM-DD')
            }
            if (element.StageEnd != null) {
              element.StageEnd = moment(new Date(element.StageEnd)).format('YYYY-MM-DD')
            }
            if (element.StageCompleted != null) {
              element.StageCompleted = moment(new Date(element.StageCompleted)).format('YYYY-MM-DD')
            }
          ...
          if (data.length > 0) {
            data.forEach((element, i) => {
              if (element.WorkStart != null) {
                element.WorkStart = moment(new Date(element.WorkStart)).format('YYYY-MM-DD')
              }
              if (element.WorkEnd != null) {
                element.WorkEnd = moment(new Date(element.WorkEnd)).format('YYYY-MM-DD')
              }
              if (element.WorkCompleted != null) {
                element.WorkCompleted = moment(new Date(element.WorkCompleted)).format('YYYY-MM-DD')
              }
              element.STT = i + 1
        ...
        <div class="flex">
          <div class="w-full">
            ...
            <div style="display: grid; grid-template-columns: repeat(2, 1fr)">
              <div class="w-full relative p-4" v-for="item in listnews" :key="item.NewsId">
                <div style="cursor: pointer">
                  <img
                    @click="openDetails(item)"
                    class="w-full"
                    :src="item.Image ? basedomainURL + item.Image : '/src/assets/image/fails.png'"
                    alt=""
                  />
                </div>
      ...
      <div class="w-4 p-3">
        <div class="surface-100 overflow-auto mt-2">
          <div class="surface-100 w-full">
            <div class="text-center">
              <a href="/yourwork" style="text-decoration: none"><h2>Công việc của bạn</h2></a>
            </div>
            <div>
